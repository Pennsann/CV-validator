...
namespace CVValidator
{
    public partial class Form1 : Form
    {
        ...
        private void btnParseCV_Click(object sender, EventArgs e)
        {
            ...
            var nameMatch = Regex.Match(cvText, @"^(?:Name|الاسم)\s*[:：]?\s*([A-Za-zأ-ي\s]{2,40})$", RegexOptions.Multiline | RegexOptions.IgnoreCase);

            var emailMatch = Regex.Match(cvText, @"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b");
            var phoneMatch = Regex.Match(cvText, @"(\+?\d[\d\s\-\(\)]{9,})");

            var skillsMatch = Regex.Matches(cvText, @"\b(C#|Java|SQL|Python|JavaScript|Power BI|Tableau|Excel|Git|R)\b", RegexOptions.IgnoreCase);
            var expMatch = Regex.Match(cvText, @"\b\d+\s+years?\b|\b\d+\s+سنوات\b", RegexOptions.IgnoreCase);

            result.Append($"Name: {(nameMatch.Success ? nameMatch.Groups[1].Value.Trim() : "Not found")}\n");
            result.Append($"Email: {(emailMatch.Success ? emailMatch.Value : "Not found")}\n");
            result.Append($"Phone: {(phoneMatch.Success ? phoneMatch.Value : "Not found")}\n");

            if (skillsMatch.Count > 0)
            {
            ...
            {
                result.Append("Skills: Not found\n");
            }

            result.Append($"Experience: {(expMatch.Success ? expMatch.Value : "Not found")}\n");
